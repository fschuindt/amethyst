defmodule Teller.GraphQL.Resolvers.AccountHistory.Month do
  ...
  @doc """
  Act as a Absinthe resolver to report account logbooks over a month
  period argument.
  ...
  defp do_resolve(%Account{} = account, args, _info) do
    with {:ok, from}   <- Date.new(args[:year], args[:month], 1),
         month_days    <- Date.days_in_month(from),
         to            <- Date.add(from, month_days),
